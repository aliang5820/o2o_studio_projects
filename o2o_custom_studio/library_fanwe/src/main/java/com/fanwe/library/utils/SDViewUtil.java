...
public class SDViewUtil
{
	...
	public static boolean isTouchView(View view, int x, int y)
	{
		...
		{
			Rect r = new Rect();
			r.left = getViewXOnScreen(view);
			r.right = r.left + getViewWidth(view);
			r.top = getViewYOnScreen(view);
	...
	{
		int[] location = getLocationOnScreen(view);
		pop.showAtLocation(view, Gravity.NO_GRAVITY, location[0], location[1] - pop.getHeight() - marginBottom);
	}
