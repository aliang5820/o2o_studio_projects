			...
			final int scrollRange, final int fuzzyThreshold, final float scaleFactor, final boolean isTouchEvent)
	{
		...
		if (view.isPullToRefreshOverScrollEnabled() && !view.isRefreshing())
		{
			...
			if (mode.permitsPullToRefresh() && !isTouchEvent && deltaValue != 0)
			{
				...
				{
					Log.d(LOG_TAG, "OverScroll. DeltaX: " + deltaX + ", ScrollX: " + scrollX + ", DeltaY: " + deltaY + ", ScrollY: " + scrollY
							+ ", NewY: " + newScrollValue + ", ScrollRange: " + scrollRange + ", CurrentScroll: " + currentScrollValue);
				}
